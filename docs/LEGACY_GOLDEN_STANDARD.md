...
**Build ir deploy:** Root `index.html` – vienintelis šaltinis; `scripts/build-locale-pages.js` generuoja `lt/index.html` ir `en/index.html`. Numatytasis public URL: `https://promptanatomy.space` su tuščiu `BASE_PATH` (`/lt/`, `/en/`); override per env `SITE_ORIGIN` / `BASE_PATH`. Žr. [DEPLOYMENT.md](../DEPLOYMENT.md).

**Sinchronizuota su kodu (2026-04-30):** skip-link „Pereiti prie turinio“, hero (Turinio DI sistema / rinkodaros vadovams, žyma „Promptų anatomija“ → `https://promptanatomy.app/`, žyma „Nemokama biblioteka“, poantraukė apie 10 CMO promptų ir ~45 min. su taisyklinga forma **„susiformuos planas ir kasdienis turinio ciklas“**, primary CTA „Pradėti nuo 1-o prompto“, antrinė nuoroda „Bendruomenė: Telegram“), hero trust pills **be** antros hero nuorodos į metodiką; po `progressIndicator` – **viena** `.cmo-provider-hub` eilutė (ChatGPT / Claude / Gemini); build `injectProviderRows` nebekartoja eilutės po kiekvienu promptu. **SEO/OG preview:** `og:image` ir `twitter:image` naudoja **self-hosted** `https://promptanatomy.space/og.png` (šaltinis `scripts/generate-og.js`, paleidžiama `npm run build` / `npm run generate:og`) su `og:image:width=1200`, `og:image:height=630`, `og:image:alt`, `twitter:image:alt` (kontraktas tikrinamas `tests/structure.test.js`); meta title/description/OG/Twitter – **EN** visoms lokelėms (CMO USA auditorija). **Nuorodų hierarchija:** `.objectives-eco-hint`, FAQ `.faq-eco-hint` → `#ecosystem-strip`; sekcija `#ecosystem-strip` (h2 `#ecosystem-strip-title`, sąrašas: metodika, Telegram, el. paštas, Leader) **prieš** `#community`; bendruomenė – tik Telegram CTA; footer – **viena** sujungta `.footer-product-link` eilutė (versija + `promptanatomy.app` + atnaujinimo data). „Kas toliau?“ – `<details>` su `summary#next-steps-jump`, viduje `.next-steps-links` su `#block1`–`#block10`. progress bar `aria-label`, info-box „Informacija: promptas N“, puslapio title; .footer-email; kalbos perjungiklis (.lang-switcher, langLtBtn, langEnBtn); kopijavimo mygtukų LT `aria-label` šablonas **„Kopijuoti promptą N į darbinių atmintinę“**; **EN build** – instrukcijų 2 eilutė (kabučių atitikmuo „ “), laiko etiketė „~3–5 min per step“, CSS .code-block::before `content`, visi 10 promptų `<pre>` turinių LT→EN; EN hero title/h1 „for Marketing Leaders“; meme `figcaption` / `img alt` ir FAQ `application/ld+json` EN locale statiniame faile; **`hreflang x-default`** įterpiamas į **`https://promptanatomy.space/en/`**. **Turinio eiliškumas (CRO 3.2):** `#framework-schema` lieka po instrukcijų ir prieš progresą; `#what-is-prompt` ir `#prompt-anatomy` (su `meme-slot-1` / `meme-slot-2`) yra **po** visų 10 promptų kortelių, prieš „Kas toliau?“; po FAQ seka `#ecosystem-strip`, tada `#community`. **privatumas.html:** „Grįžti“ nuorodos turi `id="back-link"` ir `id="back-link-footer"`; inline skriptas nustato `href` pagal `document.referrer` (lt/, en/ arba index.html), kad išsaugotų locale.
