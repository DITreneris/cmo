...
### Pataisyta
- **[Commerce/QA] Merchant listing `Offer` warnings (Ambition A / R1-support):** Search Console „Prekybininkų įrašai“ nekritinės problemos `validFrom` + `shippingDetails` uždarytos EN JSON-LD. `Offer.validFrom` = naujas `product.priceValidFrom` ([`config/sot.json`](config/sot.json): Starter/Pro `2026-05-17`, Complete `2026-05-31` — realios kainų įsigaliojimo datos, ne build data). `shippingDetails` aprašo skaitmeninį pateikimą: `shippingRate` **0 USD**, `handlingTime` / `transitTime` **0 d.** `geo.merchantCountries` (`US, GB, DE, FR, LT`) – vienas šaltinis `shippingDestination` ir `hasMerchantReturnPolicy.applicableCountry` (anksčiau `applicableCountry` buvo hardcode'intas `US`). `aggregateRating` / `review` sąmoningai **neįdėti** (nėra realių atsiliepimų; tai lieka nekritinis GSC warning). Kainos, Stripe linkai ir `/en/` UI nepakeisti. Does not exit R1. No SemVer bump. [`scripts/geo-surfaces.js`](scripts/geo-surfaces.js), [`docs/AGENT_SOT.md`](docs/AGENT_SOT.md), [`tests/structure.test.js`](tests/structure.test.js).
- **[QA/UI] Spine display-id mismatch (Ambition A / R1-support):** EN `applyStaticLocaleText` keys spine cards by `pre.id` (1/2/3/5), not display `.number` (1–4) — Improve no longer overwritten by Pro #4 video body. Dropped `Start here`, 10-card `promptData`, and dead `renderEditHints` / `renderPromptTags` / `initPromptCollapse` / PDF lightbox / lang-btn wiring. Structure + e2e guards. Does not exit R1. No SemVer bump.
- **[Docs/Orchestrator] Docs hygiene (Ambition A / R1-support):** Agent reading paths aligned with live `/en/`. `roadmap.md` R3 exit = keep hero → open `#creative-brief` (not Prompt 1 Copy); placement before `#pro-contents`. `MUST_TODO_STRIPE` storefront = **2** cards + Pro text link; repo-prep label no longer „Phase 1a“. `memo_pdf` §3.1 + env include Bundle; memo demoted to architecture reference; Commerce SSOT = GO_LIVE → MUST_TODO → `sot.json`. INDEX Active vs Reference/parked; QA_STANDARTAS free-surface pointer; todo CTA *Build my prompt*; `.cursorrules` stamp. Does not exit R1. No SemVer bump.
- **[UI/QA] Dual-CSS minimum strip (Ambition A / R1-support):** Root `index.html` `<style>` no longer restates the DS 1.6.1 hero card (`border-radius: 20px`), 1160px `.container`, pill `.trust-pill`, or leftover `.lang-switcher` / `.badge-spinoff`. `components.css` stays the one source. Full inline-`<style>` deletion and template epic stay deferred. Does not exit R1. No SemVer bump.
- **[UI/QA] Public slash URLs for privacy and terms (Ambition A / R1-support):** Public URLs are `/en/privacy/` and `/terms/` (disk files stay `.html`). Root-absolute `/styles` and `/favicon` on those pages. Vercel rewrite + 308; local `serve.json` for e2e. Storefront, footer, canonical, sitemap, `llms.txt`, and fulfillment emails use slash. `/success.html` and `/coming-soon.html` stay `.html`. No `cleanUrls`. CI pa11y serves `public/` without `-s` (wait-on `/en/`). LT hreflang/English links advertise `/en/privacy/` (not the `.html` 308). Export writes `public/en/privacy/index.html` and `public/terms/index.html` so Pages slash URLs resolve without `vercel.json`. Host-relative `/styles/` on Pages under `/cmo` stays known. Does not exit R1. No SemVer bump.
- **[UI/Docs/QA] Frontpage rules hygiene (Ambition A / R1-support):** Storefront trust uses `/en/privacy/` and `/terms/#paid-pdf-license`. LEGACY matches shipped EN (`#cb-builder` open; `Click to copy` without emoji; STYLEGUIDE **1.6.1**). `llms.txt` hubs lead with `#creative-brief`. Dead CSS removed (cycle-stepper, provider hub, comparison table). Storefront badge ink not gold; tier/amount weight 700. Does not exit R1. No SemVer bump.
- **[UI] No empty strip above Prompt Anatomy (Ambition A / R1-support):** `.container` leftover `padding-top: 60px` sat above the sticky `.header-bar` after the nav lift (100vw only broke out sideways). Top padding is 0; bar is flush with the viewport. Does not exit R1. No SemVer bump.
- **[UI] Sticky `#siteNav` actually persists (Ambition A / R1-support):** `/en/` lifts `.header-bar` + `#siteNav` out of `.header` (`overflow: hidden` was killing sticky). Page-level `position: sticky`; hash offsets on `#creative-brief` / `#pdf-storefront` / `#block1`. Labels unchanged (Workflows · Brief builder · Pricing). LT still strips `#siteNav`. Does not exit R1. No SemVer bump.
- **[UI] EN footer/ecosystem cleanup (Ambition A / R1-support):** `/en/` strips workbook chrome (`#footerSignoff`, `#footerPlaceholderHint`, `.footer .tags`). Ecosystem list = methodology only; Telegram stays `#community`; email stays footer; Leader stays `.ecosystem-strip-related` (github.io). No EN `.cmo-footer-crosslink`. Copyright drops “Training material”; Privacy `href="/en/privacy/"`. Entity line + UTM unchanged. LT snapshot unchanged. Does not exit R1. No SemVer bump.
- **[UI] EN conversion-surface bugs (Ambition A / R1-support):** Builder Context/Visual/Text steps hide again (`.cb-panel[hidden] { display: none !important }` — `display: grid` had overridden `[hidden]`). Footer methodology is written to `#footer-product-link` only (no `qa('.footer p')[0]` overwrite of sister-kit/version). Runtime **removes** leftover workbook nodes (`footer > h3`, classless hint `p`, `.tags`, `.cmo-footer-crosslink`) instead of `hidden`. Live `/en/` still duplicated Remember + Methodology because that p-index write overwrote injectFooterSuite. Does not exit R1. No SemVer bump.
- **[Docs/QA] SOT ↔ fulfillment IDs (Parked / R1-support):** [`config/sot.json`](config/sot.json) `publicId` / `downloadFileName` now match [`api/_lib/fulfillment.js`](api/_lib/fulfillment.js) `PRODUCTS` (`cmo-starter-pdf`, `prompt-anatomy-cmo-starter.pdf`, …). Tests assert equality. Stripe `metadata.product`, Blob paths, and buyer download names unchanged. Dangling `$schema` (missing `sot.schema.json`) removed. Does not exit R1. No SemVer bump.
- **[Docs] Public README EN + docs↔code (Parked / R1-support):** GitHub [`README.md`](README.md) is English and matches the live `/en/` path (brief → 2 cards → library 1/2/3/5). DEPLOYMENT Vercel `buildCommand` / Pages `BASE_PATH` match [`vercel.json`](vercel.json) and [`deploy.yml`](.github/workflows/deploy.yml). INDEX inventory includes GO_LIVE + pdf-source README. Does not exit R1. No SemVer bump.
- **[QA] Lean repo cut (Ambition A):** Paid PDF HTML interiors (`docs/pdf-source/cmo-*.html`) gitignored (operator-local; see [`docs/pdf-source/README.md`](docs/pdf-source/README.md)). `public/` no longer copies `data/`; GitHub Pages artifact is `public/` not the repo root. Live Stripe session/event/plink ids redacted in [`memo_pdf.md`](memo_pdf.md) §7. Does not exit R1. No SemVer bump. Does not change `/en/` UI or checkout.
...
### Pakeista
- **[Content/UI] OG share card (Ambition A / R1-support):** `og.png` headline *Build an image-ready prompt*; subline *Brief builder · 4 free workflows* — no price. Type uses display/UI stacks from tokens; drop-shadow removed. `ogImageAlt` matches. Page `<title>` / meta description still may name kits. Does not exit R1. No SemVer bump.
- **[Docs/Orchestrator] Agents v1.5.8 lesson §10.22:** Vercel `click_checkout` + `sku` visoms storefront SKU; do not restore `click_starter`. No SemVer bump.
- **[UI] Checkout events visiems SKU (Ambition A / R1-support):** `js/va-track.js` siunčia `click_checkout` su `data.sku` (`starter` \| `bundle` \| `pro`). Complete ir Pro klikai nebenyksta. Jokio PII. `click_starter` pašalintas. Does not exit R1. No SemVer bump.
- **[UI] EN hero sample still (Ambition A / R1-support):** `/en/` `.hero-sample-image` is the `0400` challenger (structured cognac bag, gold clasp, travertine, empty plaster). Filenames stay `assets/hero/brief-sample-satori.png` + `.webp` (tests lock the string). Caption/alt unchanged. Operator candidates stay `assets/hero_png/` (gitignored). Does not exit R1. No SemVer bump.
- **[UI] EN conversion-surface cleanup (Ambition A / R1-support):** Hero copy is Brief → Prompt (`Build my prompt` / `See pricing`; trust without `$3.99`). Builder: shell-width + 17px type; `Try an example` by the lead; readiness copy without “weak”. EN footer hides rocket + placeholder + chips; Telegram outline + *Join the community.*; ecosystem links ink. IDs/hrefs/Stripe unchanged. Does not exit R1. No SemVer bump.
