...
- **[UI] EN conversion-surface bugs (Ambition A / R1-support):** Builder Context/Visual/Text steps hide again (`.cb-panel[hidden] { display: none !important }` — `display: grid` had overridden `[hidden]`). Footer methodology is written to `#footer-product-link` only (no `qa('.footer p')[0]` overwrite of sister-kit/version). Does not exit R1. No SemVer bump.
